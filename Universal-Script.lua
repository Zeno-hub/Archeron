-- Archeron Hub - Multi-Game Version 
...
local function getText(key)
    local translations = {
        ...
        -- Game 2 Features
        autoGrassCollect = "Auto Grass Collect",
        autoGrassCollectDesc = "Collect all grass types",
        autoPop = "Auto Pop",
        autoPopDesc = "Automatically increase pop",
        upgradeAll = "Upgrade All",
        upgradeAllDesc = "Upgrade trees + auto max programmer",
        autoProgrammer = "Auto Programmer",
        autoProgrammerDesc = "Auto upgrade programmer to max level",
...
-- Multi-Game Configs
local gameConfigs = {
    -- GAME 1: Sticks Incremental
    ["120870800305934"] = {
        gameName = "Sticks Incremental",
    ...
    -- GAME 2: Grass Incremental Simulator
    ["133086043677134"] = {
        gameName = "[🌱] Grass Incremental Simulator",
        menus = {"menuMain", "menuPlayer", "menuInfo"},
        features = {
            menuMain = {
                {name = "autoGrassCollect", type = "toggle"},
                {name = "autoPop", type = "toggle"},
                {name = "autoProgrammer", type = "toggle"},
                {name = "upgradeAll", type = "island_upgrade"},
            },
            menuPlayer = {
                {name = "fly", type = "toggle"},
                {name = "noclip", type = "toggle"},
        ...
        remotePaths = {
            grassCollect = "ReplicatedStorage.Remotes.GrassCollect",
            increasePop = "ReplicatedStorage.Remotes.IncreasePop",
            upgradeTree = "ReplicatedStorage.Remotes.UpgradeTree",
        },
    },
}
...
local gameConfig = gameConfigs[currentGameId]

if not gameConfig then
    warn("⚠️ GAME NOT SUPPORTED!")
    warn("📋 Current PlaceId:", currentGameId)
    player:Kick("❌ Archeron Hub - Game Not Supported\n\n📋 Place ID: " .. currentGameId .. "\n\n💡 This game is not in our supported list.\n\nRequest support in our Discord!")
    return
end

print("🎮 Detected Game:", gameConfig.gameName)
...
-- Replica Module (untuk Programmer)
local ReplicatedStorage = game:GetService("ReplicatedStorage")
local Replica = nil
pcall(function()
    Replica = require(ReplicatedStorage.Modules.Shared.Replica)
end)
...
GameNameLabel.Text = gameConfig.gameName
...
-- Programmer Cost Calculator (dari ModuleScript)
local function getProgrammerCost(level)
    if not Replica then return 100000 end
    ...
    pcall(function()
        if Replica.Data.Upgrades.Tree14 >= 1 then
            discount = 0.1
        end
        if Replica.Data.Upgrades.Tier >= 3 then
            tierMultiplier = 0.001
        end
    end)
    
    return math.floor(baseCost * (multiplier ^ level) * discount * tierMultiplier)
end
...
local function createToggle(name, parent)
    ...
    btn.Text = featureStates[name] and "ON" or "OFF"
    ...
    btn.MouseButton1Click:Connect(function()
        ...
        if featureStates[name] then
            ...
            -- GAME 2 FEATURES
            elseif name == "autoGrassCollect" then
                print("🔄 Activating autoGrassCollect...")
                if not RemoteEvents.grassCollect then 
                    connectRemote("grassCollect", gameConfig.remotePaths.grassCollect) 
                    wait(0.5) 
                end
                if RemoteEvents.grassCollect then
                    print("✅ Starting grass collect loop")
                    loop = task.spawn(function()
                        while featureStates[name] do
                            pcall(function()
                                local collectAmounts = {
                                    normal = 100,
                                    ruby = 100,
                                    silver = 100,
                                    golden = 100,
                                ...
                                RemoteEvents.grassCollect:FireServer(collectAmounts)
                            end)
                            task.wait(0.1)
                        end
                    end)
                else
                    print("❌ RemoteEvents.grassCollect not found!")
                end
            
            elseif name == "autoPop" then
                print("🔄 Activating autoPop...")
                if not RemoteEvents.increasePop then 
                    connectRemote("increasePop", gameConfig.remotePaths.increasePop) 
                    wait(0.5) 
                end
                if RemoteEvents.increasePop then
                    print("✅ Starting pop loop")
                    loop = task.spawn(function()
                        while featureStates[name] do
                            pcall(function()
                                local args = {false}
                                RemoteEvents.increasePop:FireServer(unpack(args))
                            end)
                            task.wait(0.1)
                        end
                    end)
                else
                    print("❌ RemoteEvents.increasePop not found!")
                end
            
            -- AUTO PROGRAMMER (Pake logic dari ModuleScript)
            elseif name == "autoProgrammer" then
                print("🔄 Activating autoProgrammer...")
                if not Replica then
                    print("❌ Replica module not found!")
                    createNotification("Auto Programmer", "Replica module not found!", 3)
                    featureStates[name] = false
                    btn.Text = "OFF"
                ...
                print("✅ Starting programmer loop")
                loop = task.spawn(function()
                    while featureStates[name] do
                        pcall(function()
                            local currentLevel = Replica.Data.Upgrades.Programmers or 0
                            local currentPop = Replica.Data.Pop or 0
                            local cost = getProgrammerCost(currentLevel)
                            
                            -- Max level check
                            if currentLevel >= 100000 then
                                print("✅ Programmer already at max level (100000)")
                                return
                            end
                            
                            -- Buy if we have enough Pop
                            if currentPop >= cost then
                                local UpgradeQueue = require(ReplicatedStorage.Modules.UpgradeQueue)
                                UpgradeQueue:Add({
                                    currencyName = "Pop",
                                    upgradeValue = "Programmers",
                                    amount = 1,
                                    cost = cost,
                                    max = 100000,
                                    autoBuy = true
                                })
                                print("💰 Bought Programmer upgrade! Level:", currentLevel + 1)
                            end
                        end)
                        task.wait(0.5)
                    end
...
-- Create Island Upgrade Feature (MULTI-SELECT + AUTO PROGRAMMER)
local function createIslandUpgrade(name, parent)
    local selectedIslands = {[1] = true} -- Multi-select support
    ...
    local islandLabel = Instance.new("TextLabel", frame)
    islandLabel.Size = UDim2.new(1, -30, 0, 20)
    islandLabel.Position = UDim2.new(0, 15, 0, 75)
    islandLabel.BackgroundTransparency = 1
    islandLabel.Text = "Select Islands (Multi-Select):"
    islandLabel.TextColor3 = Color3.fromRGB(200, 200, 220)
    ...
    scrollFrame.Size = UDim2.new(1, -30, 0, 105)
    ...
    local islandList = Instance.new("UIListLayout", scrollFrame)
    islandList.Padding = UDim.new(0, 5)
    
    local islandPad = Instance.new("UIPadding", scrollFrame)
    islandPad.PaddingTop = UDim.new(0, 5)
    islandPad.PaddingLeft = UDim.new(0, 5)
    islandPad.PaddingRight = UDim.new(0, 5)
    islandPad.PaddingBottom = UDim.new(0, 5)
    
    -- Island Configuration
    local islandConfig = {
        {island = 1, treeType = 1, trees = 50},
        {island = 2, treeType = 1, trees = 50},
        {island = 3, treeType = 2, trees = 50},
    }
    
    local islandButtons = {}
    
    -- Create island buttons with multi-select
    for i = 1, 3 do
        local islandBtn = Instance.new("TextButton", scrollFrame)
        islandBtn.Size = UDim2.new(1, -10, 0, 35)
        islandBtn.BackgroundColor3 = selectedIslands[i] and Color3.fromRGB(138, 43, 226) or Color3.fromRGB(40, 40, 60)
        islandBtn.BorderSizePixel = 0
        islandBtn.Text = "Island " .. i
        islandBtn.TextColor3 = Color3.fromRGB(255, 255, 255)
        islandBtn.Font = Enum.Font.GothamBold
        islandBtn.TextSize = 13
        Instance.new("UICorner", islandBtn).CornerRadius = UDim.new(0, 6)
        
        islandButtons[i] = islandBtn
        
        islandBtn.MouseButton1Click:Connect(function()
            -- Toggle selection
            selectedIslands[i] = not selectedIslands[i]
            islandBtn.BackgroundColor3 = selectedIslands[i] and Color3.fromRGB(138, 43, 226) or Color3.fromRGB(40, 40, 60)
        end)
    end
    
    scrollFrame.CanvasSize = UDim2.new(0, 0, 0, islandList.AbsoluteContentSize.Y + 10)
    
    local upgradeBtn = Instance.new("TextButton", frame)
    upgradeBtn.Size = UDim2.new(1, -30, 0, 35)
    upgradeBtn.Position = UDim2.new(0, 15, 1, -45)
    upgradeBtn.BackgroundColor3 = Color3.fromRGB(100, 200, 100)
    upgradeBtn.BorderSizePixel = 0
    upgradeBtn.Text = "START UPGRADE"
    ...
    local programmerLoop = nil
    local isUpgrading = false
    
    upgradeBtn.MouseButton1Click:Connect(function()
        isUpgrading = not isUpgrading
        
        if isUpgrading then
            upgradeBtn.Text = "STOP UPGRADE"
            upgradeBtn.BackgroundColor3 = Color3.fromRGB(200, 100, 100)
            
            -- Count selected islands
            local selectedCount = 0
            for _, selected in pairs(selectedIslands) do
                if selected then selectedCount = selectedCount + 1 end
            end
            
            if selectedCount == 0 then
                createNotification("Upgrade All", "Please select at least one island!", 3)
                isUpgrading = false
                upgradeBtn.Text = "START UPGRADE"
                upgradeBtn.BackgroundColor3 = Color3.fromRGB(100, 200, 100)
                return
            ...
            local islandNames = {}
            for i, selected in pairs(selectedIslands) do
                if selected then table.insert(islandNames, "Island " .. i) end
            end
            
            createNotification("Upgrade All", "Upgrading: " .. table.concat(islandNames, ", "), 3)
            
            if not RemoteEvents.upgradeTree then 
                connectRemote("upgradeTree", gameConfig.remotePaths.upgradeTree) 
                wait(0.5) 
            end
            
            -- AUTO PROGRAMMER LOOP
            if Replica then
                print("✅ Starting auto programmer loop")
                programmerLoop = task.spawn(function()
                    while isUpgrading do
                        pcall(function()
                            ...
                            if currentLevel < 100000 and currentPop >= cost then
                                local UpgradeQueue = require(ReplicatedStorage.Modules.UpgradeQueue)
                                UpgradeQueue:Add({
                                    currencyName = "Pop",
                                    upgradeValue = "Programmers",
            ...
            -- TREE UPGRADE LOOP
            if RemoteEvents.upgradeTree then
                print("✅ Starting upgrade loop for selected islands")
                upgradeLoop = task.spawn(function()
                    while isUpgrading do
                        for islandNum, selected in pairs(selectedIslands) do
                            if not selected or not isUpgrading then continue end
                            
                            local config = islandConfig[islandNum]
                            if config then
                                for treeNum = 1, config.trees do
                                    ...
                                    pcall(function()
                                        ...
                                        RemoteEvents.upgradeTree:FireServer(unpack(args))
                                    end)
                                    task.wait(0.03)
                                end
                            end
                ...
                print("❌ RemoteEvents.upgradeTree not found!")
                isUpgrading = false
                upgradeBtn.Text = "START UPGRADE"
                upgradeBtn.BackgroundColor3 = Color3.fromRGB(100, 200, 100)
            end
            ...
            upgradeBtn.Text = "START UPGRADE"
            upgradeBtn.BackgroundColor3 = Color3.fromRGB(100, 200, 100)
            createNotification("Upgrade All", "Stopped upgrading", 3)
            if upgradeLoop then upgradeLoop = nil end
            if programmerLoop then programmerLoop = nil end
        end
    end)
end
...
local function loadMenu(key)
    ...
    if features then
        for _, f in ipairs(features) do
            ...
            elseif f.type == "island_upgrade" then
                createIslandUpgrade(f.name, ContentArea)
            end
        end
    end
...
print("🎮 Game:", gameConfig.gameName)
